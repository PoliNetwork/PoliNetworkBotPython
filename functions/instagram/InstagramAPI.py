...
class PyInstagramAPI:
    ...
    def login(self, force=False):
        if not self.isLoggedIn or force:
            ...
            a1 = self.SendRequest('si/fetch_headers/?challenge_type=signup&guid=' + self.generateUUID(False), None,
                                  True)
            if a1:
                data = {
                ...
                a2 = self.SendRequest('accounts/login/', self.generateSignature(json.dumps(data)), True)
                if a2:
    ...
    def SendRequest(self, endpoint, post=None, login=False):
        ...
        if type(self.IGDataPath) is None:
            pathCookies = '/cookies/'
        else:
            pathCookies = str(self.IGDataPath) + '/cookies/'
